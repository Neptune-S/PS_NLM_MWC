                                  ...
                                  uint32_t mbox_index )
{
    ...
    if( NULL == pAviHndlr )
    {
        log_info( "ERR: AVI library INIT not done\n\r",
                 mbox_index );
    ...
    if( mbox_index > 1 )
    {
        log_info( "ERR: Invalid MBOX index [%d]\n\r",
                 mbox_index );
                                    ...
                                    uint32_t mbox_index )
{
    ...
    if( 0 == mbox_index )
    {
        log_dbg( "Read VSPA MBOX[%d],handle 0x%x Q : 0x%x, 0x%x\n\r",
                 mbox_index, AviHndlr, ( ( struct avi_hndlr * ) AviHndlr )->VspaToCm4QMbox0 , mbox);
    ...
    if( IN_32( &pVspaRegs->vspa_status ) & VSPA_MBOX0_STATUS )
    {
     //  log_dbg( "%s: Rcvd mbox0 from VSPA\n\r", __func__ );
        ...
        xHigherPriorityTaskWoken = pdTRUE;
  	    ...
  	    log_dbg( "%s: Rcvd mbox0 0x%x \n\r", __func__ ,  vspambox.msb);

       if( pdTRUE != xQueueSendToBackFromISR( AviHndlr->VspaToCm4QMbox0,
                                               &vspambox, &xHigherPriorityTaskWoken ) )
        {
