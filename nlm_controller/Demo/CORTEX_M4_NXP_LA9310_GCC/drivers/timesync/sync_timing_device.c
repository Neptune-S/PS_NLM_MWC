...
const char * SyncTimingDeviceModeNames[ 3 ] = { "Bootloader", "Application", "Invalid" };
...
static SyncStatus_t prvSyncTimingDeviceWaitForCTS( SyncTimingDeviceContext_t * pxContext )
{
    ...
    while( 1 )
    {
        returnStatus = prvSyncTimingDeviceTrxCommand( pxContext, &ucBuff, 1, eSyncReadTransaction );

        if( returnStatus == eSyncStatusSuccess )
        {
            if( ucBuff & SYNC_TIMING_DEVICE_CLEAR_TO_SEND )
            {
                return returnStatus;
            }
        }
                                        ...
                                        struct la9310_std_fwupgrade_data *pxFwInfo )
{
    ...
    for( i = 0; i < pxFwInfo->fwcount; i++ )
    {
        ...
        while( ulIndex < pxFwInfo->fwinfo[lDloadOrder[i]].size )
        {
            ...
            log_info( "addr: 0x%x index: %d chunk size: %d\r\n",
